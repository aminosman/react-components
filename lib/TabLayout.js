...
function TabLayout(_a) {
    ...
    var getInitialPinnedTabs = function () {
        ...
        try {
            ...
            if (stored) {
                var storedIds_1 = JSON.parse(stored);
                if (Array.isArray(storedIds_1)) {
                    // Convert stored IDs to boolean array
                    return nav.map(function (x) { return storedIds_1.includes(x.id); });
                }
            }
        }
        catch (e) {
    ...
    var savePinnedTabsToStorage = function (tabs) {
        ...
        try {
            ...
            var safeTabs_1 = tabs.length === nav.length
                ? tabs
            ...
            var pinnedIds = nav.map(function (x, i) { return (safeTabs_1[i] ? x.id : null); }).filter(function (id) { return id !== null; });
    ...
    (0, react_1.useEffect)(function () {
        ...
        try {
            var stored = localStorage.getItem(pinnedTabsStorageKey);
            if (stored) {
                var storedIds_2 = JSON.parse(stored);
                if (Array.isArray(storedIds_2)) {
                    // Convert stored IDs to boolean array
                    var loadedPinnedTabs = nav.map(function (x) { return storedIds_2.includes(x.id); });
